...
11. **Engine Continuity Principle.**
   > **Engine owns execution; PAIOS owns continuity.**
   AI engines execute (code generation, conversation, debugging, shell
   commands).  PAIOS maintains continuity (Task persistence, Handoff
   standards, Knowledge governance, cross-engine scheduling).
   Execution artifacts (sessions, internal logs, engine-private formats)
   belong to each engine.  PAIOS never stores full execution logs — only
   the index references and handoff context needed for continuity across
   engines.
...
任何新增平台层（如 `50_RUNTIME`、新顶层目录）必须遵循以下流程才能进入平台基线：
...
架构评审
    ↓
合并到平台基线（Case-01 / Fleet Release）
```

**规则**：
1. **任何 Case 都可以开发**；只有经过平台治理确认的变更，才能成为 PAIOS Core 的一部分。
2. 新增平台层必须先在至少一个开发实例完成**跨引擎真实验证**，再经架构评审后才能进入平台基线。
3. 验证标准由对应的 ADR 在 `Path to Accepted` 中定义。
4. 此规则不限制新增子目录（如 `40_AUTOMATION/` 下新增子目录），仅限制新增顶层平台层。
