...
3. 电脑A：`git push`
4. 电脑B：`git clone <remote> ${PAIOS_DRIVE}:/PAIOS`
...
电脑A
│
├── ${PAIOS_DRIVE}:/PAIOS              ← GitHub 同步
│   ├── 50_DATA/          ← 本地运行时（不同步）
│   └── 70_TMP/           ← 本地缓存（不同步）
│
└── ${PAIOS_DRIVE}:/QuarkSync/DATA     ← 夸克云盘自动同步
        ├── 项目档案/      ← 按项目分类（需填写项目清单）
        ├── 个案档案/      ← 文件名含「个案」「案主」
        ├── 工作报告/      ← 文件名含「月报」「报告」「总结」
        ├── 财务资料/      ← 文件名含「财务」「发票」「报销」
...
```bash
git clone https://github.com/leiwanfung626-cmyk/paios.git ${PAIOS_DRIVE}:/PAIOS
```

### 3️⃣ 等待夸克同步

夸克云盘同步目录（例如 `${PAIOS_DRIVE}:/QuarkSync/DATA`）同步完成后，两台电脑的业务数据完全一致。

### 4️⃣ 创建本地运行时目录

```bash
mkdir ${PAIOS_DRIVE}:/PAIOS/50_DATA
mkdir ${PAIOS_DRIVE}:/PAIOS/70_TMP
```
...
```
打开电脑 → 等待夸克同步 → cd ${PAIOS_DRIVE}:/PAIOS → git pull → 继续工作
```

## 最佳实践

- **业务数据**：统一放在夸克同步目录下（如 `${PAIOS_DRIVE}:/QuarkSync/DATA`），PAIOS 只引用不管理
...
> 将文件丢到 `${PAIOS_DRIVE}:/QuarkSync/DATA` 根目录，脚本自动归类到对应中文文件夹。

**脚本位置**：`${PAIOS_DRIVE}:/PAIOS/40_AUTOMATION/05_SCRIPTS/classify_files.py`

### 用法

```bash
# 试运行（先看看会怎么分，不实际移动）
python ${PAIOS_DRIVE}:/PAIOS/40_AUTOMATION/05_SCRIPTS/classify_files.py --dry-run

# 实际分类
python ${PAIOS_DRIVE}:/PAIOS/40_AUTOMATION/05_SCRIPTS/classify_files.py
```
...
在 `${PAIOS_DRIVE}:/PAIOS/QuarkSync/PROJECT_LIST.md` 中填写项目名，脚本会自动识别以项目名开头的文件。格式：
