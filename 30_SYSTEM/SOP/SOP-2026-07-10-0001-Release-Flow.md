...
- **状态**：Active（v1.0.1 首次启用）
- **日期**：2026-07-10
- **Owner**：Developer（Evan）
- **关联**：`ADR-0014`（升级机制）· `ADR-0015`（聚合与查看）· `ADR-0016`（架构基线）· `RELEASES/1.0.1.md` · `RELEASES/upgrade-receipt-1.0.1.md` · `F:\Fleet\README.md`

---

## 一句话

每次发版 = **Developer 提交并打 Tag → 用户 git pull 升级并回传 Manifest → Developer 拉取后人工生成 Fleet 周报**。Fleet 不回推用户。
...
2. **User2 `git pull`** → 升级（填 `profile.yaml` / 跑 `collect_manifest.py`）→ 生成 manifest（按 `instance.id` 命名 `case-02.yaml`）→ `commit` 仅 `PAIOS-Usage/` → `push`。
3. **User3** 同上（生成 `case-03.yaml`）。
4. **Developer `git pull`** → 取得三个 manifest → 复制到 `F:\Fleet\manifests\` → 人工聚合为 `fleet.md`（存 `F:\Fleet\reports\`）→ `commit` 到 Fleet 目录。
