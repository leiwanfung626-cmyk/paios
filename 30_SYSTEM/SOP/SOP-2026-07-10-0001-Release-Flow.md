...
每次发版 = **Developer 提交并打 Tag → 用户 git pull 升级并 publish Manifest（out-of-band）→ Developer 收集到 Fleet → 人工生成 Fleet 周报**。Fleet 不回推用户。
...
2. **User2 `git pull`** → 升级（填 `profile.yaml` / 跑 `collect_manifest.py`）→ 生成 `PAIOS-Usage/manifest.yaml`（含 `manifest_version`）→ `publish` → `F:\Fleet\incoming\`（out-of-band，不 push Core）。
...
- **Push 范围**：**User 对 Core 仓库永不 push**（ADR-0018 角色模型）——User 仅 `git pull` 读平台。Manifest 经 out-of-band publish 回流 Fleet，不经 Core 仓库。Workspace（知识 / 项目 / 照片）**不入库、不推送**。（Phase A 靠提交范围约束；Phase B 物理 Core/Workspace 拆分后由结构保证；Role Model 进一步从权限上消除写 Core 能力。）
