# ADR-0003: 生命周期层目录架构（Lifecycle Layer Directory Architecture）

## Status
Accepted

> **2026-07-12 修正**：架构已从 7 层扩展至 00-90 十层模型。
> 新增 `80_HISTORY/`（工程历史档案）作为独立生命周期层。
> 完整生命周期：00 捕获 → 10 工作 → 20 知识 → 30 系统 → 40 自动化 → 50 数据 → 60 外部 → 70 临时 → 80 历史 → 90 归档。
> 本 ADR 为历史基线文档，当前完整模型以 ADR-INDEX.md 为准。
...
数字前缀表达生命周期阶段：00 捕获 → 10 工作 → 20 知识 → 30 系统 → 40 自动化 → 50 数据 → 60 外部 → 70 临时 → 80 历史 → 90 归档。
...
- 80_HISTORY 为工程历史提供独立生命周期阶段，避免与归档混淆
